...
/* tag::reference[]
= Using the `MSGPACK_HW_SIMD_*` predefs
...
int main()
{
#if defined(MSGPACK_HW_SIMD_AVAILABLE)
...
compare it. Those predef are of the form `MSGPACK_HW_SIMD_"ARCH"` (where `"ARCH"`
is either `ARM`, `PPC`, or `X86`). For example, if you compile code for x86
architecture, you will have to use `MSGPACK_HW_SIMD_X86`. Its value will be the
...
int main()
{
#if MSGPACK_HW_SIMD_X86 >= MSGPACK_HW_SIMD_X86_SSE3_VERSION
...
int main()
{
#if MSGPACK_HW_SIMD_X86 >= MSGPACK_HW_SIMD_X86_SSE2_VERSION &&\
    MSGPACK_HW_SIMD_X86 <= MSGPACK_HW_SIMD_X86_SSSE3_VERSION
